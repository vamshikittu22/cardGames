...
export const GameLog: React.FC<GameLogProps> = ({ logs, isOpen, onClose }) => {
  return (
    <div className={`
      fixed top-0 right-0 h-full w-80 bg-[#111827] border-l border-white/10 z-[300] shadow-2xl transform transition-transform duration-500 pointer-events-auto
      ...
      <div className="p-6 h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-2 scrollbar-hide pb-10">
          {logs.slice().reverse().map((log) => (
            <div key={log.id} className="bg-white/5 border border-white/5 p-4 rounded-xl flex flex-col gap-1 animate-in slide-in-from-right-2">
          ...
          {logs.length === 0 && (
             <div className="flex flex-col items-center justify-center h-full opacity-20 gap-4 text-center">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest">No moves chronicled</p>
             </div>
          )}
        </div>
      </div>
