...
export const Board: React.FC<BoardProps> = ({ room, currentPlayerId, onUpdateRoom }) => {
  ...
  const startClashWindow = (card: IGameCard, cost: number, type: any, targetInfo?: any) => {
    onUpdateRoom({
      ...room,
  ...
  const executeFinalizedAction = (action: any) => {
    ...
    if (type === 'intro') {
      ...
      startShaknyWindow({ type: 'power', card, playerId: actorId });
    } else if (type === 'maya') {
      setActiveMaya(card);
    ...
    onUpdateRoom({
      ...
      interruptStatus: type === 'intro' ? { type: 'shakny-window', endTime: Date.now() + 3000, rollDetails: { type: 'power', card, playerId: actorId } } : undefined,
  ...
  // Fix: Added resolveMaya handler
  const resolveMaya = () => {
    if (!activeMaya) return;
    showToast(`${activeMaya.name} effect has manifested!`, "#2563EB");
    // Maya cards go to submerge pile after use
    onUpdateRoom({
      ...room,
  ...
  // Fix: Added handleAssuraSelection handler
  const handleAssuraSelection = (id: string) => {
    if (targetingMode !== 'capture-assura') return;
    setSelectedAssuraId(id);
    setTargetingMode('capture-majors');
    showToast("Select your Majors to launch the assault.", "#EA580C");
  };
  ...
  const handleClash = () => {
    ...
    showToast("Duel with a Clash card!", "#DC2626");
  };

  const handleShakny = () => {
    setTargetingMode('shakny-select');
    showToast("Modify fate with Shakny!", "#EA580C");
  };
  ...
  const onInterruptCardSelect = (cardId: string) => {
    ...
    if (card.type === 'Clash') {
      ...
      setTargetingMode('none');
      showToast(`${card.name}: ${value > 0 ? '+' : ''}${value}`, "#EA580C");
    }
  };
  ...
  const handleDiceComplete = (result: number) => {
    ...
    if (type === 'power') {
      ...
      onUpdateRoom({ ...room, players: updatedPlayers, drawDeck: updatedDrawDeck, shaknyModifiers: [], gameLogs: [...room.gameLogs, { id: generateId(), turn: room.currentTurn, playerName: room.players.find(p => p.id === playerId)?.name || '', action: `invoked ${card.name}: Result ${finalResult} (Roll: ${result}, Shakny: ${shaknyBonus})`, kpSpent: 0, timestamp: Date.now() }] });
    } else {
      ...
      onUpdateRoom({ ...room, players: updatedPlayers, assuras: updatedAssuras, assuraReserve: updatedReserve, submergePile: updatedSubmerge, shaknyModifiers: [], gameLogs: [...room.gameLogs, { id: generateId(), turn: room.currentTurn, playerName: currentPlayer.name, action: `Assura capture: Result ${finalResult}`, kpSpent: 2, timestamp: Date.now() }] });
    }
    setRollingFor(null);
  };
  ...
  const handleDrawCard = () => {
    if (!isMyTurn || currentPlayer.karmaPoints < 1) return;
  ...
  return (
    <div className="fixed inset-0 bg-[#0F172A] text-white overflow-hidden flex flex-col select-none">
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm animate-in fade-in flex flex-col items-center justify-center pointer-events-none">
          <div className="bg-black/80 px-8 py-4 rounded-full border-2 border-yellow-500/50 flex items-center gap-4 animate-bounce">
            ...
            <p className="text-sm font-black uppercase tracking-widest text-white">
              {targetingMode === 'clash-select' ? "Select a Clash card to use" :
               ...
               targetingMode === 'capture-majors' ? "Select attackers" : "Select target"}
            </p>
          ...
          {targetingMode === 'capture-majors' && (
             <div className="mt-8 pointer-events-auto">
                <button onClick={handleConfirmCaptureAttack} disabled={selectedAttackerIds.length === 0} className={`px-12 py-4 rounded-xl text-xl font-black uppercase shadow-2xl transition-all ${selectedAttackerIds.length > 0 ? 'bg-red-600' : 'bg-gray-600 cursor-not-allowed opacity-50'}`}>Launch Attack</button>
             </div>
          )}
        </div>
      )}
      ...
      {activeMaya && (
        <div className="fixed inset-0 z-[200] bg-black/90 flex flex-col items-center justify-center gap-8 animate-in zoom-in">
           ...
           <div className="flex gap-4">
             <button onClick={() => { resolveMaya(); setActiveMaya(null); }} className="px-12 py-4 bg-blue-600 rounded-xl text-xl font-black uppercase hover:bg-blue-500 transition-all">Manifest Effect</button>
      ...
      <main className="flex-1 relative mt-24 mb-32 flex flex-col items-center justify-between p-4 md:p-8 overflow-hidden">
        <div className="w-full flex justify-center items-center gap-4 flex-wrap max-h-[30vh]">
          {otherPlayers.map((p, idx) => (
            <PlayerArea key={p.id} player={p} isActive={activePlayer.id === p.id} isCurrent={false} position={getOpponentPosition(idx, otherPlayers.length)} targetingMode={targetingMode} onTargetSelect={handleTargetSelection} />
          ))}
        </div>
        ...
        <div className="w-full flex justify-center">
          <PlayerArea 
            ...
            targetingMode={targetingMode} onTargetSelect={handleTargetSelection} selectedAttackerIds={selectedAttackerIds}
          />
        </div>
      </main>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 max-w-7xl mx-auto px-4">
        <ControlPanel 
          kp={currentPlayer.karmaPoints} isActive={isMyTurn} onAction={handleAction} onEndTurn={handleEndTurn} actionsUsed={room.actionsUsedThisTurn} deckEmpty={room.drawDeck.length === 0 && room.submergePile.length === 0}
          isInterrupting={!!room.interruptStatus}
        />
      </div>
